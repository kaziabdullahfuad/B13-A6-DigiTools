...
const DigitalTools = () => {
    ...
    return (
        ...
        <div className=''>
            <div className=' w-10/12 md:mx-auto flex flex-col justify-center items-center gap-4 mt-30'>
                ...
                <div className='flex bg-white border-[#F6F6F6] border rounded-full gap-2 justify-center items-center p-2 shadow-md'>
                    ...
                    <button onClick={()=>setactiveBtn('cart')} className={`py-3 px-5 ${activeBtn==='cart' ? 'bg-linear-to-r from-[#4F39F6] to-[#9514FA] text-white rounded-full gap-1 font-medium' : 'text-[#25065D] font-medium'}`}>Cart(2)</button>
                </div>
                
                {activeBtn==='product' ? <Products></Products> : <FullCart></FullCart>}
